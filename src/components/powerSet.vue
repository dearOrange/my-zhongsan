<template>
	<div class="powerCon">
		<div class="powerHead">
			<Icon type="android-lock" size="20" style="margin-right:6px;"></Icon>用户组权限设置
		</div>
			...
			<div class="sectionLeft">
				...
				<ul @click='setFormItem($event)'>
					<li v-for='(formItem,index) in formItemData' :key='index' :index='index'>
						<Checkbox v-model='single[index]' :label="formItem.tfDesc" :name="formItem.id" @on-change="changePar"></Checkbox>{{formItem.tfDesc}}
					</li>
				</ul>
			</div>
		...
		methods: {
			setFormItem (e) {
				...
				this.$nextTick(() => {
		        	...
		        	var startRole = roleFunction.split(',');
	    			this.checkChild(CheckboxItems, startRole);
				})
			},
			powerSet () {
				var groupId = localStorage.getItem("token1");
				this.ajax({
					method:'post',
					url:this.apiUrl + '/role/updateByGroupId',
					data:JSON.stringify({
						"groupId":groupId,
			...
			check (CheckboxItem, startRole) {
        		for(let i = 0,len=CheckboxItem.length;i<len;i++){
        			for(let o = 0;o<startRole.length;o++){
        				if(CheckboxItem[i].getAttribute('name') == startRole[o] ){
			...
			checkChild (CheckboxItems, startRole) {
				for(let i = 0,len=CheckboxItems.length;i<len;i++){
        			for(let o = 0;o<startRole.length;o++){
        				if(CheckboxItems[i].getAttribute('name') == startRole[o] ){
			...
			changePar (value) {
				if(value == true){
//					console.log(this.$data.formItemData)
				}
			}
		},
        mounted() {
        	...
        	this.ajax({
        		...
        		that.$nextTick(function() {
        			...
        			var roleFunction = localStorage.getItem("token2");
	        		var startRole = roleFunction.split(',');
	        		that.check(CheckboxItem, startRole);
	        		that.setFormItem();
        		})
